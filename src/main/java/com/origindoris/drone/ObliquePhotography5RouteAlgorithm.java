...
public class ObliquePhotography5RouteAlgorithm {
    ...
    private static final Map<String, double[]> BOUNDS_CACHE = new ConcurrentHashMap<>();
    ...
    private static class SpatialGrid {
        ...
        private String getCellKey(LatLngPoint point) {
            double yMeters = point.latitude * 111000.0;
            double xMeters = point.longitude * 111000.0 * Math.cos(Math.toRadians(point.latitude));
            long x = (long) Math.floor(xMeters / cellSize);
            long y = (long) Math.floor(yMeters / cellSize);
            return x + "," + y;
        }
    }
    ...
    private static boolean isPointInBoundingBox(LatLngPoint point, List<LatLngPoint> polygon) {
        double[] bounds = getBoundsCached(polygon);
    ...
    private static double[] getBoundsCached(List<LatLngPoint> polygon) {
        String key = String.valueOf(polygon.hashCode());
        double[] cached = BOUNDS_CACHE.get(key);
        if (cached != null) {
            return cached;
        }
        ...
        double[] bounds = new double[]{minLat, maxLat, minLng, maxLng};
        BOUNDS_CACHE.put(key, bounds);
        return bounds;
    }
        ...
        for (int i = 0; i < optimization.directions.length; i++) {
            ...
            if (!routeResult.waypoints.isEmpty()) {
                ...
                if (!effectiveWaypoints.isEmpty()) {
                    ...
                    int size = effectiveWaypoints.size();
    ...
    private static List<LatLngPoint> ensureCounterClockwise(List<LatLngPoint> polygon) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            signedArea += p1.longitude * p2.latitude - p2.longitude * p1.latitude;
        }
        if (signedArea < 0) {
    ...
    private static List<LatLngPoint> findUncoveredAreas(List<LatLngPoint> originalPolygon,
        ...
        // 使用并行流处理大量采样点
        java.util.stream.Stream<LatLngPoint> stream = samplePoints.size() > 1000
                ? samplePoints.parallelStream()
                : samplePoints.stream();
        return stream
                .filter(samplePoint -> waypoints.stream()
                        .noneMatch(waypoint -> calculateDistanceFast(samplePoint, waypoint) <= coverageRadius))
    ...
    private static double calculateEdgeCoverageRate(List<RouteDirection> routes, List<LatLngPoint> originalPolygon, ObliqueParams params) {
        ...
        for (LatLngPoint edgePoint : edgeSamplePoints) {
            ...
            for (LatLngPoint waypoint : allWaypoints) {
                if (calculateDistanceFast(edgePoint, waypoint) <= coverageRadius) {
                    covered = true;
                    break;
                }
            }
    ...
    public static double debugArea(List<LatLngPoint> polygon) {
    ...
    public static int debugNearbyCount(List<LatLngPoint> points, LatLngPoint center, double radius) {
        SpatialGrid grid = new SpatialGrid(radius);
        for (LatLngPoint p : points) {
            grid.addPoint(p);
        }
        return grid.getNearbyPoints(center, radius).size();
    }
}
